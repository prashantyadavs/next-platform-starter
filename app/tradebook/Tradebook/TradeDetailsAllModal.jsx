...
export default function TradeDetailsAllModal({ selectedDate,trade, }) {
...
console.log(selectedTrade)
  ...
  const paginatedTrades = useMemo(() => {
    ...
    return trades.slice(startIndex, startIndex + rowsPerPage);
  }, [page, trades]);
  ...
  // Fetch all trades on modal open
  useEffect(() => {
    if (isOpen) {
      fetchAllTrades(); // Fetch all trades when modal is opened
    }
  }, [isOpen]);
  const fetchAllTrades = async () => {
    try {
        ...
        const response = await fetch(`https://main.tradifyapi.in/trades/user/${encodeURIComponent(userEmail)}`);
        ...
        // Sort trades by entryDate from latest to oldest
        tradesData.sort((a, b) => new Date(b.entryDate) - new Date(a.entryDate));
    
        setTrades(tradesData); // Set the fetched and sorted trades
      } catch (error) {
...
const formatRatio = (ratio) => {
  ...
  return (
    <>
      ...
      <Modal 
        ...
        backdrop="blur"
        size="3xl"
        className="dark z-5000000" // Add the z-index here
        isOpen={isOpen}
                ...
                <TableBody>
                  {paginatedTrades.map((trade) => {
                    const avatarSrc = `https://images.5paisa.com/MarketIcons/${
                      trade.symbol.includes("NIFTY") ? "NIFTY50" : trade.symbol.replace("-", "&")
                    }.png`;
                    ...
                    const chipColor = totalProfit >= 0 ? statusColorMap.profit : statusColorMap.loss;
                    const chipLabel = totalProfit >= 0 ? `+ ₹${totalProfit.toFixed(2)}` : `- ₹${Math.abs(totalProfit).toFixed(2)}`;
                      ...
                      <TableRow key={trade._id}>
                        ...
                        <TableCell>
                          <Chip
                            ...
                            color={trade.isLongTrade ? statusColorMap.buy : statusColorMap.sell}
                            size="sm"
                            variant="flat"
                          >
  ...
  {(() => {
    ...
    // Use toLocaleDateString with UTC-based values
    ...
    return new Date(Date.UTC(utcYear, utcMonth, utcDay)).toLocaleDateString('en-GB', options);
  })()}
</TableCell>
